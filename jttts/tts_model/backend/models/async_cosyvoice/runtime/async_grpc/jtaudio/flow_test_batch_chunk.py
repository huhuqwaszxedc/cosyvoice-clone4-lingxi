...
token_0 = flow_input_data_0['token'][:1,...][:, 0:23]                # [1, 51]          torch.int64
...
token_1 = flow_input_data_1['token'][:1,...][:, 0:43]                # [1, 64]
token_len_1 = flow_input_data_1['token_len'][:1,...]                 # [1]
token_len_1[0] = 43

from torch.nn.utils.rnn import pad_sequence
tokens = pad_sequence(
    [token_0.squeeze(0), token_1.squeeze(0)],
    batch_first=True,
    padding_value=0
)
tokens_len = torch.zeros([2], dtype = torch.int32).to(device)
tokens_len[0] = token_len_0
tokens_len[1] = token_len_1
prompt_token = prompt_token.repeat(2, 1)
prompt_token_len = prompt_token_len.repeat(2)
prompt_feat = prompt_feat.repeat(2, 1, 1)
prompt_feat_len = prompt_feat_len.repeat(2)
embedding = embedding.repeat(2, 1)
finalizes = finalizes.repeat(2)

tts_mel, _  = flow_model.inference_chunck(
                token=tokens,
